  ...
  async handle(handlerInput) {
    ...
    if(level>input.INPUT_DATA.length){
      ...
      if(sessionAttributes.gameState === 'ENDED' || sessionAttributes.gameState === 'CLUES' || (sessionAttributes.questionCount == 0)){
        let question = 0; 
        speechArr.push(`You Level ${level.toString()} is about ${battle.Battle}`);
        ...
        speechArr = []
        speechArr.push("Hope you got the overview of battle");
...
const CancelAndStopIntentHandler = {
  ...
  async handle(handlerInput) {
    ...
    let speechArr = [];
    speechArr.push("Ok Thanks for Playing Battle of Brains.");
